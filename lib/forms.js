...
function limpiaForm(formulario){
    $(formulario).find(".is-valid").removeClass().addClass('form-control').addClass('requerido');
    $(formulario).find(".is-invalid").removeClass().addClass('form-control').addClass('requerido');
}
//Detecta la clase frm-new y genera el formulario dinamico asociado al form-id 
...
//Detecta la clase quiz-new y genera el cuestionario dinamico asociado al form-id
//Mismo funcionamiento y declaraciones que los formualrios dinamicos
function detectarCuestionario() {

	$(".quiz-new").each(function () {
		$(this).load(
			frmUrl + "/obtenerNuevoCuestionario/" + this.dataset.form,
			function () {
                //Inicializo STEPPER
